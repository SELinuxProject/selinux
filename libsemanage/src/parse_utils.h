...
extern int parse_init(semanage_handle_t *handle, const char *filename,
...
extern int parse_assert_ch(semanage_handle_t *handle, parse_info_t *info,
			   char ch);

/* Throw an error if the specified string
 * does not follow is not found, otherwise
 * eat the string */
extern int parse_assert_str(semanage_handle_t *handle, parse_info_t *info,
...
int parse_fetch_int(semanage_handle_t *hgandle, parse_info_t *info, int *num,
		    char delim);
...
extern int parse_fetch_string(semanage_handle_t *handle, parse_info_t *info,
			      char **str_ptr, char delim, int allow_spaces);
