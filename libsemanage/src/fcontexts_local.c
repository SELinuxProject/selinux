...
int semanage_fcontext_iterate_local(
	...
	int (*handler)(const semanage_fcontext_t *record, void *varg),
	void *handler_arg)
{
	dbase_config_t *dconfig = semanage_fcontext_dbase_local(handle);
...
static int validate_handler(const semanage_fcontext_t *fcon, void *varg)
{
	...
	if (con && sepol_context_check(handle->sepolh, policydb,
...
invalid:
	if (semanage_context_to_string(handle, con, &str) >= 0) {
		ERR(handle, "invalid context %s specified for %s [%s]", str,
		    expr, type_str);
		free(str);
	} else
		ERR(handle, "invalid context specified for %s [%s]", expr,
		    type_str);
	return -1;
}
