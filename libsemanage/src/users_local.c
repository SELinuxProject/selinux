...
static int lookup_seuser(semanage_handle_t *handle,
			 const semanage_user_key_t *k)
{
	...
	semanage_seuser_list_local(handle, &records, &count);
	for (i = 0; i < count; i++) {
...
int semanage_user_count_local(semanage_handle_t *handle, unsigned int *response)
{
	dbase_config_t *dconfig = semanage_user_dbase_local(handle);
