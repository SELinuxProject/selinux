...
int semanage_seuser_key_create(semanage_handle_t *handle, const char *name,
			       semanage_seuser_key_t **key_ptr)
{
	semanage_seuser_key_t *tmp_key =
		(semanage_seuser_key_t *)malloc(sizeof(semanage_seuser_key_t));
			  ...
			  semanage_seuser_t **seuser_ptr)
{
	...
	if (seuser->mls_range &&
	    (semanage_seuser_set_mlsrange(handle, new_seuser,
					  seuser->mls_range) < 0))
		goto err;
