...
CORPUS_MIN_POL_HASH=$(sha1sum checkpolicy/fuzz/min_pol.mls.conf | awk '{ print $1 }')
cp checkpolicy/fuzz/min_pol.mls.conf "checkpolicy/fuzz/$CORPUS_MIN_POL_HASH"
zip -j "$OUT/checkpolicy-fuzzer_seed_corpus.zip" "checkpolicy/fuzz/$CORPUS_MIN_POL_HASH"
