...
{
	fprintf(stderr,
		"usage:  %s [-r role] [-l level] [-s service] [-v] user [fromcon]\n",
		name);
	if (detail)
		fprintf(stderr, "%s:  %s\n", name, detail);
	exit(rc);
...
int main(int argc, char **argv)
{
	...
	if (!is_selinux_enabled()) {
		fprintf(stderr, "%s may be used only on a SELinux kernel.\n",
			argv[0]);
		free(level);
		free(role);
		free(service);
	...
	if ((argc - optind) < 2) {
		if (getcon(&cur_con) < 0) {
			fprintf(stderr,
				"%s:  couldn't get current context:  %s\n",
	...
	if (role != NULL && role[0])
		...
		ret = get_default_context_with_level(seuser, level, cur_context,
						     &usercon);
	if (ret) {
