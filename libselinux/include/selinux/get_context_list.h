...
extern int get_ordered_context_list(const char *user, const char *fromcon,
				    char ***list);
...
/* As above, but use the provided MLS level rather than the
   default level for the user. */
extern int get_default_context_with_level(const char *user, const char *level,
...
extern int get_default_context_with_role(const char *user, const char *role,
					 const char *fromcon, char **newcon);
...
/* Given a list of authorized security contexts for the user, 
   ...
   Caller must free via freecon.
   Returns 0 on success or -1 otherwise. */
extern int query_user_context(char **list, char **newcon);

/* Allow the user to manually enter a context as a fallback
   if a list of authorized contexts could not be obtained. 
   Caller must free via freecon.
   Returns 0 on success or -1 otherwise. */
extern int manual_user_enter_context(const char *user, char **newcon);
