...
struct selabel_handle {
	...
	bool (*func_get_digests_all_partial_matches)(
		struct selabel_handle *h, const char *key,
	...
	struct selabel_lookup_rec *(*func_lookup_best_match)(
		struct selabel_handle *h, const char *key, const char **aliases,
		int type);
 ...
 * Validation function
 */
extern int selabel_validate(struct selabel_lookup_rec *contexts);
...
extern void
	__attribute__((format(printf, 1, 2))) (*myprintf)(const char *fmt, ...);

#define COMPAT_LOG(type, fmt...)                \
	do {                                    \
		if (myprintf_compat)            \
			myprintf(fmt);          \
		else                            \
...
extern int compat_validate(const struct selabel_handle *rec,
 ...
 */
extern int read_spec_entries(char *line_buf, size_t nread, const char **errbuf,
			     int num_args, ...);
