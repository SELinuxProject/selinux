...
static inline int mls_range_glblub(struct mls_range *dst,
				   const struct mls_range *r1,
				   const struct mls_range *r2)
{
	...
	if (ebitmap_and(&dst->level[0].cat, &r1->level[0].cat,
			&r2->level[0].cat) < 0) {
		return -1;
	}

	if (ebitmap_and(&dst->level[1].cat, &r1->level[1].cat,
			&r2->level[1].cat) < 0) {
		return -1;
	}
...
static inline int mls_range_eq(const struct mls_range *r1,
			       const struct mls_range *r2)
{
...
extern int mls_semantic_level_cpy(mls_semantic_level_t *dst,
