...
extern int sepol_compute_av_reason_buffer(
	sepol_security_id_t ssid, sepol_security_id_t tsid,
	sepol_security_class_t tclass, sepol_access_vector_t requested,
	struct sepol_av_decision *avd, unsigned int *reason, char **reason_buf,
	unsigned int flags);
 ...
 */
extern int sepol_validate_transition_reason_buffer(
	sepol_security_id_t oldsid, sepol_security_id_t newsid,
 ...
 */
extern int
sepol_context_to_sid(sepol_const_security_context_t scontext, /* IN */
 ...
 */
extern int sepol_get_user_sids(sepol_security_id_t callsid, char *username,
 ...
 */
extern int sepol_port_sid(uint16_t domain, uint16_t type, uint8_t protocol,
 ...
 */
extern int sepol_ibpkey_sid(uint64_t subnet_prefix_p, uint16_t pkey,
 ...
 * `dev_name', and `port'.
 */
extern int sepol_ibendport_sid(char *dev_name, uint8_t port,
			       sepol_security_id_t *out_sid);
 ...
 * interface.
 */
extern int sepol_netif_sid(char *name, sepol_security_id_t *if_sid,
 ...
 */
extern int sepol_node_sid(uint16_t domain, void *addr, size_t addrlen,
			  sepol_security_id_t *out_sid);
