...
static int expand_attributes_in_attributes(sepol_handle_t *handle,
					   policydb_t *p)
{
	...
	while (!done && reps < p->p_types.nprim) {
		...
		ebitmap_for_each_positive_bit(&attrs, ni, i) {
			...
			if (ebitmap_match_any(&td->types, &attrs)) {
				...
				ebitmap_init(&types);
				ebitmap_for_each_positive_bit(&td->types, nj,
							      j) {
					if (i == j) {
						ERR(handle,
						    "Found loop in type attributes involving: %s",
...
static int role_dominates_copy_callback(hashtab_key_t key
...
static int role_types_copy_callback(hashtab_key_t key, hashtab_datum_t datum,
				    void *data)
{
	char *id;
	role_datum_t *role, *new_role;
...
static int role_roles_copy_callback(hashtab_key_t key, hashtab_datum_t datum,
				    void *data)
{
	char *id;
	role_datum_t *role, *new_role;
...
static int expand_role_attributes_in_attributes(sepol_handle_t *handle,
						policydb_t *p)
{
	...
	while (!done && reps < p->p_roles.nprim) {
		...
		ebitmap_for_each_positive_bit(&attrs, ni, i) {
			...
			if (ebitmap_match_any(&rd->roles, &attrs)) {
				...
				ebitmap_init(&roles);
				ebitmap_for_each_positive_bit(&rd->roles, nj,
							      j) {
					if (i == j) {
						ERR(handle,
						    "Found loop in role attributes involving: %s",
...
int role_set_expand(role_set_t *x, ebitmap_t *r, policydb_t *out,
		    __attribute__((unused)) policydb_t *base, uint32_t *rolemap)
{
	unsigned int i;
	ebitmap_node_t *rnode;
		  ...
		  int verbose, int check)
{
	...
	for (curblock = state.base->global; curblock != NULL;
		...
		if (hashtab_map(decl->p_types.table, attr_convert_callback,
				&state))
			goto cleanup;
		if (hashtab_map(decl->p_types.table, type_bounds_copy_callback,
				&state))
			goto cleanup;
	}
	...
	if (hashtab_map(state.base->p_roles.table, role_bounds_copy_callback,
			&state))
		goto cleanup;
	if (hashtab_map(state.base->p_roles.table, role_dominates_copy_callback,
			&state))
		goto cleanup;
	if (hashtab_map(state.base->p_roles.table, role_types_copy_callback,
			&state))
		goto cleanup;
	if (hashtab_map(state.base->p_roles.table, role_roles_copy_callback,
			&state))
		goto cleanup;
	for (curblock = state.base->global; curblock != NULL;
	     curblock = curblock->next) {
		...
		if (decl == NULL)
			continue;
		if (hashtab_map(decl->p_roles.table, role_bounds_copy_callback,
				&state))
			goto cleanup;
		if (hashtab_map(decl->p_roles.table,
				role_dominates_copy_callback, &state))
			goto cleanup;
		if (hashtab_map(decl->p_roles.table, role_types_copy_callback,
				&state))
			goto cleanup;
		if (hashtab_map(decl->p_roles.table, role_roles_copy_callback,
				&state))
			goto cleanup;
	}
	...
	for (curblock = state.base->global; curblock != NULL;
		...
		if (hashtab_map(decl->p_users.table, user_copy_callback,
				&state))
			goto cleanup;
		if (hashtab_map(decl->p_users.table, user_bounds_copy_callback,
				&state))
			goto cleanup;
	}
